<?php if(!defined('APP_PATH')||!defined('JWCOM_PATH')){exit('Access Denied');} ?>
...
function doyocms_upload_config(){
	...
	var bgimage='include/js/jwcomupload/add.gif';
...
<?php if($fileover==1){
	...
	$(".info").empty();
	<?php } ?>
	$(".info").append('<li><p class="loading"><img src="include/js/jwcomupload/loading.gif" /></p><p class="file"><span>'+date+'</span><br /><span class="progress"></span></p><p class="cancel"><a onclick="doyocms_upload_gocancel('+"'"+date+"'"+',1);"></a></p></li>');
<?php } ?>
};
...
.list li .cancel a{float:left; width:28px; height:25px; cursor:pointer; background:url(include/js/jwcomupload/cancel.gif) no-repeat 0 5px;}
...
.info .cancel a{float:left;width:28px;height:28px; cursor:pointer; background:url(include/js/jwcomupload/cancel.gif) no-repeat 0 6px;}
<?php } ?>
</style>
</head>
...
<body>
<div class="swf">
<object id="upload_swf" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="80" height="26"><param name="movie" value="include/js/jwcomupload/jwcom_upload.swf?date=<?php echo time(); ?>" /><param name="quality" value="high" /><param name="wmode" value="opaque" /><param name="swfversion" value="9.0.0" /><!--[if !IE]>--><embed src="include/js/jwcomupload/jwcom_upload.swf?date=<?php echo time(); ?>" name="upload_swf" quality="high" wmode="opaque"  swfversion="9.0.0" type="application/x-shockwave-flash"  width="80" height="26"></embed><!--<![endif]--></object>
</div>
<?php if($fileover==2){ ?>
<ul class="list"></ul>
