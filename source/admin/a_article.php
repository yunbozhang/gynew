...
class a_article extends syController
{
	function __construct(){
		...
		if(($this->syArgs('a',1)=='add'||$this->syArgs('a',1)=='edit')&&$this->syArgs('run')==1){
			...
			$this->newrow2 = array(
				'body' => code_body($this->syArgs('body',4)),
			);
				...
				if(false == $newVerifier){
					...
					if(syDB($this->molds.'_field')->create($this->newrow2)){
						if($GLOBALS['G_DY']['rewrite']["rewrite_open"]!=1&&syExt('site_html')==1&&$this->newrow1['isshow']==1&&$this->newrow1['mrank']==0&&$this->newrow1['mgold']==0){
							...
							$body=array_filter(explode("[jwcom|page]",$this->newrow2['body']));
							$allb=count($body);
							if($allb>1){
								for ($i = 1; $i <= $allb; $i++) {
									if($i>1){
				...
				if(false == $newVerifier){
					...
					if($edit_field){
						if($GLOBALS['G_DY']['rewrite']["rewrite_open"]!=1&&syExt('site_html')==1&&$this->newrow1['isshow']==1&&$this->newrow1['mrank']==0&&$this->newrow1['mgold']==0){
							...
							$this->chtml->c_molds('article',array('id'=>$this->syArgs('id')),$c_html_f);
							$body=array_filter(explode("[jwcom|page]",$this->newrow2['body']));
							$allb=count($body);
							if($allb>1){
								for ($i = 1; $i <= $allb; $i++) {
									if($i>1){
									$this->chtml->c_molds('article',array('id'=>$this->syArgs('id'),'page'=>$i),str_replace('.',$i.'.',$c_html_f));
									}
								}
							}
						}
