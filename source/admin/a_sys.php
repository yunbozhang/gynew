...
class a_sys extends syController
{
	...
	function index(){
		if($this->syArgs('run')==1){
			...
			@fclose($fp_tp);
			$config=array('http_path','site_title','site_keywords','site_description','site_comtitle','site_phone','site_comemail','site_combeian','site_address','site_jwcdnapi','view_themes','site_html','site_html_dir','site_html_rules','site_html_index','cache_auto','cache_time','filetype','filesize','imgwater','imgwater_t','imgcaling','img_w','img_h','comment_audit','comment_user','rewrite_open','enable_gzip','enable_gzip_level');
			foreach($config as $v){
