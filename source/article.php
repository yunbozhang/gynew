...
class article extends syController
{
	...
	function index(){
		...
		$body=array_filter(explode("[jwcom|page]",$this->article['body']));
		if(count($body)>1){
			$pages=array(
		...
		$this->type=syDB('classtype')->find(" molds='article' and tid=".$this->article['tid']." ",null,'tid,classname,classnamesub,litpic,keywords,description,t_content,htmldir,htmlfile,mrank,msubmit');
