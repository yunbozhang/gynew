...
{	
	...
	function login(){
		if($this->syArgs("go")==1){
			if($this->syArgs("user",1) && $this->syArgs("pass",1)){
				...
				if(!$this->syArgs("vercode",1)||md5(strtolower($this->syArgs("vercode",1)))!=$_SESSION['jwcom_verify'])message("验证码错误");
				}
	...
	function reg(){
		...
		if($this->syArgs("go")==1){
			...
			if($this->syArgs('pass1',1)!=$this->syArgs('pass2',1))message("两次密码输入不一致");
			if($GLOBALS['G_DY']['vercode']==1){
			if(md5(strtolower($this->syArgs("vercode",1)))!=$_SESSION['jwcom_verify']){message("验证码错误");}
			}
			$newrow1 = array(
				'user' => $this->syArgs('user',1),
