...
class product extends syController
{
	...
	function index(){
		...
		$body=array_filter(explode("[jwcom|page]",$this->product['body']));
		if(count($body)>1){
			$pages=array(
		...
		$this->type=syDB('classtype')->find(" molds='product' and tid=".$this->product['tid']." ",null,'tid,classname,classnamesub,litpic,keywords,description,t_content,htmldir,htmlfile,mrank,msubmit,unit');
