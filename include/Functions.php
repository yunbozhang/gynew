...
function syError($msg, $output = TRUE, $stop = TRUE){
	...
	if(TRUE != SP_DEBUG){
		...
		echo "<body>程序出错！<br><br>出现此情况可能为：1、模板标签书写错误。2、修改过程序代码，代码修改有误。<br><br>请在后台系统设置中开启PHP错误调试，或者修改include\inc.php第一行'mode' => 'debug'后查看信息错误记录<br><br>您可以购买官方商业服务，为您提供全程服务。</body></html>";
		if(TRUE == $stop)exit;
	}
...
function dykeycode($string, $operation = 'DECODE'){
	$ckey_length = 4;
	$key = md5($GLOBALS['G_DY']['ext']['secret_key'] != '' ? $GLOBALS['G_DY']['ext']['secret_key'] : 'JWCOM_secret_key');
...
//统计
function  total_page($sql,$v='JWCOM_total_page'){
