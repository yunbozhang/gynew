	...
	private function template_html_loop($f){
		...
		switch($db){
			...
			case $dbleft.'product':
				...
				$txt.='foreach($table'.$as.' as $'.$as.'){ $'.$as.'["tid_leafid"]=$sy_class_type->leafid($'.$as.'["tid"]);$'.$as.'["n"]=$'.$as.'n=$'.$as.'n+1; $'.$as.'["url"]=html_url("product",$'.$as.'); $'.$as.'["title"]=stripslashes($'.$as.'["title"]);$'.$as.'["description"]=stripslashes($'.$as.'["description"]); $'.$as.'["description"]=stripslashes($'.$as.'["description"]); ?>';
			break;
			case $dbleft.'message':
			...
			case $dbleft.'classtype':
				$field_all='tid,molds,pid,classname,classnamesub,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow';
				foreach($a as $k=>$v){
					if($k=='not'&&$a['pid']){
						$not=1;
					}else{
				...
				$txt.='foreach($table'.$as.' as $'.$as.'){ $'.$as.'["tid_leafid"]=$sy_class_type->leafid($'.$as.'["tid"]);$'.$as.'["n"]=$'.$as.'n=$'.$as.'n+1; $'.$as.'["classname"]=stripslashes($'.$as.'["classname"]);$'.$as.'["classnamesub"]=stripslashes($'.$as.'["classnamesub"]);$'.$as.'["description"]=stripslashes($'.$as.'["description"]); $'.$as.'["url"]=html_url("classtype",$'.$as.'); ?>';
			break;
			case $dbleft.'special':
